...
public:
    ...
    static std::string format(const std::string& format, Args ... args)
    {
        ...
        if (size_s <= 0 )
            return format + ". Error during formatting";
                   ...
                   const calendar_t cal)
{
    ...
    if (!is_valid())
    {
        std::string s = str::format("Year: %d, month: %d, day: %d, hour: %d, minute: %d, second: %d",
...
std::string datetime::to_string(const std::string& format)
{
    ...
    tm.tm_min = minute();
    tm.tm_sec = second();
    const std::string msec_pattern = "%f";
    size_t pos = format.find(msec_pattern);
    std::string fmt = format;
    if (pos != std::string::npos)
        fmt = fmt.replace(pos, msec_pattern.size(), std::to_string(millisecond()));
    std::stringstream buf;
    buf << std::put_time(&tm, fmt.c_str());
    return buf.str();
}
